...
import { HttpStatusCode } from '@/helpers/axiosInstance'
...
export function useNuxtForm<TFormData extends TFormDataBase>(data: TFormData): FormWrapper<TFormData>

export function useNuxtForm<TFormData extends TFormDataBase>(initialData: TFormData): FormWrapper<TFormData> {
  ...
  const form = reactive({
    ...
    async submit<TResponse = any>(
      ...
      const body = transform(this.data())
